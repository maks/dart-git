...
class MockClone extends Clone {
  ...
  HttpFetcher getHttpFetcher(ObjectStore store, String origin, String url,
    ...
    MockHttpFetcher fetcher =
        new MockHttpFetcher(store, origin, url, username, password);
...
defineTests() {
  group('git.nonetwork.clone', () {
    test('invalid repo', () {
      ...
      return clone.clone().catchError((e) {
        if (e is GitException) return true;
        throw e;
      });
    });
  ...
  test('valid repo ending with .git', () {
    ...
    GitOptions options = new GitOptions(
        ...
        store: new ObjectStore(dir));
    MockClone clone =
        new MockClone(options, VALID_REPO_URL_ENDING_WITH_DOT_GIT);
    return clone.clone().then((String result) {
      return (result == "success");
    });
  });
  ...
  test('valid repo not ending with .git', () {
    ...
    GitOptions options = new GitOptions(
        ...
        store: new ObjectStore(dir));
    MockClone clone =
        new MockClone(options, VALID_REPO_URL_NOT_ENDING_WITH_DOT_GIT);
    return clone.clone().then((String result) {
      return (result == "success");
    });
  });
