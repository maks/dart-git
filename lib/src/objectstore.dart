...
class ObjectStore {
  ...
  Future clearRemoteRefs() {
    return root
        .getDirectory(GIT_REFS_REMOTES_ORIGIN_PATH)
  ...
  Future<String> getHeadForRef(String headRefName) {
    return FileOps.readFileText(_rootDir, gitPath + headRefName)
        .then((content) {
  ...
  Future<CommitPushEntry> _getCommits(
      GitRef remoteRef, Map<String, bool> remoteShas, String sha) {
    List commits = [];
      ...
      return dirEntry.createFile(objectFileName).then((File fileEntry) {
        return fileEntry.file().then((File file) {
          Future<String> writeContent() {
            ArrayBuffer content =
                new ArrayBuffer.fromBytes(Zlib.deflate(store).data);
