...
class Commit {
          ...
          return Status.updateAndGetStatus(store, entry)
            ...
            if (status.type != FileStatusType.UNTRACKED) {
              ...
              if (status.type == FileStatusType.STAGED ||
                ...
                return fileEntry.readBytes().then((ArrayBuffer buf) {
  ...
  static Future createCommit(
      GitOptions options, String parent, String treeSha, String refName) {
    ObjectStore store = options.store;
    ...
    return store
        .writeRawObject(ObjectTypes.COMMIT_STR, commitContent.toString())
        .then((String commitSha) {
