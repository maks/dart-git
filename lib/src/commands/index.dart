...
class Index {
  ...
  void updateIndexForEntry(Entry entry, FileStatus status) {
        ...
        switch (oldStatus.type) {
          ...
          default:
            throw new GitException(
                GitErrorConstants.GIT_FILE_STATUS_TYPE_UNKNOWN,
  ...
  FileStatus getStatusForEntry(Entry entry) => _statusIdx[entry.fullPath];
  ...
  Future updateIndex([bool updateSha = true]) {
    return walkFilesAndUpdateIndex(_store.root, updateSha)
        .then((List<FileStatus> statuses) {
    ...
    return FileOps.listFiles(root).then((List<ChromeFileEntry> entries) {
      ...
      return Future.forEach(entries, (Entry entry) {
        ...
        if (fileStatuses.isEmpty) {
          deleteIndexForEntry(root.fullPath);
        } else if (fileStatuses
            .any((status) => status.type != FileStatusType.COMMITTED)) {
...
class FileStatus {
  ...
  static FileStatus createFromEntry(Entry entry) {
    FileStatus status = new FileStatus();
  ...
  static FileStatus createForDirectory(Entry entry) {
    FileStatus status = new FileStatus();
    status.path = entry.fullPath;
